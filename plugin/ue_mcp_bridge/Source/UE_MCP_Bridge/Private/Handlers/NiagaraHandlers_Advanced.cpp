...
TSharedPtr<FJsonValue> FNiagaraHandlers::SetDynamicInput(const TSharedPtr<FJsonObject>& Params)
{
	...
	UNiagaraScript* DynScript = LoadAssetByPath<UNiagaraScript>(DynamicInputScript);
	if (!DynScript)
	{
		return MCPError(FString::Printf(
...
namespace
{
		...
		FString& OutContext, TSharedPtr<FJsonValue>& OutError)
	{
		...
		OutSystem = LoadAssetByPath<UNiagaraSystem>(SystemPath);
		if (!OutSystem)
		{
			OutError = MCPAssetLoadError(SystemPath, TEXT("UNiagaraSystem"));
			return nullptr;
		}
		FString ResolveError;
