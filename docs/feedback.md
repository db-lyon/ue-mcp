...
**Status:** the hosted endpoint is not deployed yet, so `author="bot"` takes the prefilled-URL path on every call today. Deployment is tracked in [#461](https://github.com/db-lyon/ue-mcp/issues/461). `author="user"`, the default, posts directly and is unaffected.
