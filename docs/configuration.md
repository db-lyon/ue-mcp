...
| `npx ue-mcp feedback prompts <on\|off>` | Stop or resume the agent offering to file feedback after an `execute_python` workaround. `disable: [feedback]` in `ue-mcp.yml` also silences them. |
| `npx ue-mcp update notify <on\|off>` | Stop or resume the "newer version available" notice. `UE_MCP_DISABLE_UPDATE_CHECK=1` does the same per process. |
