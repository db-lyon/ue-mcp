...
| `set_motion_matching_chooser` | Drive the Motion Matching node's Database from a ChooserTable so the database is selected at runtime by character state. Wires a thread-safe EvaluateChooser (result typed to PoseSearchDatabase) into the node's Database pin; the chooser's context object defaults to the anim instance (Self). Params: `assetPath (AnimBP), chooserPath (ChooserTable), graphName? (default AnimGraph)` |
