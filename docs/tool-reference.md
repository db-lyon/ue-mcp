...
| `delete_actors` | Bulk-delete actors. Params: `at least one of labelPrefix, className, tag, classPathContains, classPathContainsAny; dryRun? to preview`. `classPathContains` is a case-insensitive substring of the generated UClass path (for example `/SurvivalGameKitV2/`). `classPathContainsAny` matches if the path contains any of several substrings. |
