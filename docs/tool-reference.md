...
| `read_graph` | Read graph nodes. Supports pagination and file dumps. `dumpToFile` writes the full graph by default, or the requested slice when `offset`/`limit` is provided. Params: `assetPath, graphName, offset?, limit?, includePins?, includeDefaults?, includeComments?, dumpToFile?, outputPath?` |
