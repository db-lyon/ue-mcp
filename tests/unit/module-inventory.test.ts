...
const PER_SESSION: Record<string, string> = {
  ...
  "target-params.ts": "Injects the per-call editor target and re-points a context at the session it names.",
...
const SESSION_INDEPENDENT: Record<string, string> = {
  ...
  "routing-params.ts": "The dispatcher's parameter names and the helper that strips `action`. A leaf with no imports and no state.",
  "action-effects.ts": "Indexes whichever tool graph the caller hands it, one index per graph; an action's effect does not change with the editor it runs in.",
  ...
  "call-envelope.ts": "Unwraps and validates one call against the tool it names; holds nothing per editor.",
