...
export const pcgTool: ToolDef = categoryTool(
  ...
  "Procedural Content Generation: graphs, nodes, connections, execution, volumes.",
  {
    ...
    unwrap_instance_nodes: bp("mutate", "Give instance nodes (UPCGSettingsInstance wrappers, read-only in the PCG editor's details panel) their own settings object, keeping every value and edge. Settings from a shared asset are copied into the node. Nodes that already own their settings are left alone. Params: assetPath, nodeName? (default: every node in the graph) (#1087)", "unwrap_pcg_instance_nodes", (p) => ({ assetPath: p.assetPath, nodeName: p.nodeName })),
