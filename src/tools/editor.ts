...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    render_sequence_frames: {
      kind: "bridge",
      effect: "mutate",
      description: "Render a Level Sequence to a PNG image sequence in ONE call: for each frame it scrubs, forces the evaluation, and captures through a transient SceneCapture2D (RGBA8 LDR, real PNG), instead of scrub_sequence + capture_scene_png per frame. Camera: the sequence's camera cuts by default (frames with no active cut are skipped and reported), or cameraActorLabel/cameraActorPath for one camera actor, or location/rotation/fov for a fixed camera. The camera's own FOV and post process are used. Range is in display frames: startFrame and endFrame (inclusive), or startSeconds and endSeconds (end exclusive); default is the playback range. frameStep skips frames. More frames than maxFrames is refused. Files are written to outputDir (absolute or project-relative, default Saved/SequenceFrames/<sequence>) as <sequence>.<frame>.png. Refused while PIE runs. The playhead is restored afterwards and playback is left paused. A run that nears the 600s timeout stops, returns truncated=true and nextStartFrame. Returns files[], frames[] with per-frame evaluateMs/captureMs/exportMs/totalMs, and timing totals. Params: sequencePath (or assetPath), startFrame? | startSeconds?, endFrame? | endSeconds?, frameStep? (default 1), maxFrames? (default 300, max 5000), width? (default 1280), height? (default 720), outputDir?, format? (png), cameraActorLabel? OR cameraActorPath?, location?, rotation?, fov?, fullyLoadTextures? (default true) (#1098)",
      bridge: "render_sequence_frames",
      timeoutMs: 600_000,
      mapParams: (p) => ({ sequencePath: p.sequencePath ?? p.assetPath, startFrame: p.startFrame, endFrame: p.endFrame, startSeconds: p.startSeconds, endSeconds: p.endSeconds, frameStep: p.frameStep, maxFrames: p.maxFrames, width: p.width, height: p.height, outputDir: p.outputDir, format: p.format, cameraActorLabel: p.cameraActorLabel, cameraActorPath: p.cameraActorPath, location: p.location, rotation: p.rotation, fov: p.fov, fullyLoadTextures: p.fullyLoadTextures }),
    },
  ...
  undefined,
  {
    ...
    componentName: z.string().optional().describe("read_bone_transforms: which SkeletalMeshComponent to read; omit for the first one (#756). get_runtime_values: root every path at the component with this instance name (#1113)"),
    ...
    cameraActorPath: z.string().optional().describe("add_sequence_section / render_sequence_frames: full object path of the camera actor; alternative to cameraActorLabel (#983)"),
    ...
    startFrame: z.number().optional().describe("render_sequence_frames: first display frame to render. Mutually exclusive with startSeconds (#1098)"),
    endFrame: z.number().optional().describe("render_sequence_frames: last display frame to render, inclusive. Mutually exclusive with endSeconds (#1098)"),
    frameStep: z.number().optional().describe("render_sequence_frames: render every Nth frame (default 1) (#1098)"),
    maxFrames: z.number().optional().describe("render_sequence_frames: refuse a range with more frames than this (default 300, max 5000) (#1098)"),
    outputDir: z.string().optional().describe("render_sequence_frames: absolute or project-relative directory for the frames (default Saved/SequenceFrames/<sequence>) (#1098)"),
    format: z.string().optional().describe("render_sequence_frames: image format; only png (#1098)"),
    cameraActorLabel: z.string().optional().describe("add_sequence_section CameraCut: camera actor to bind. render_sequence_frames: camera actor to render through instead of the camera cuts (#548)"),
    ...
    // #1078: deliberately not "action". The category tool dispatches on a
    // parameter of that name, so a handler parameter sharing it is
    // unreachable: the surface strips the key on its way to the bridge, and a
    // caller who writes it selects an action instead of answering a dialog.
    dialogAction: z.enum(["escape", "close"]).optional().describe(
      "respond_to_dialog: dismiss the dialog WITHOUT pressing one of its buttons, for a modal offering no button that fits. The editor then carries on down its unanswered-prompt path. Prefer buttonLabel whenever a button says what you mean"),
    ...
    commitDeletes: z.boolean().optional().describe("save_dirty: use the editor's dirty-package save, which deletes the packages of deleted World Partition actors and reports written/deleted files (default false) (#1156)"),
