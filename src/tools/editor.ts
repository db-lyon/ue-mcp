...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    execute_command: bp("mutate", "Run console command. Params: command", "execute_command"),
    execute_python: {
      kind: "handler",
      effect: "mutate",
    ...
    set_cvars: bp("mutate", "Bulk-set console variables. Params: cvars ({name: value} object OR [{name, value}] array). Returns per-cvar old/new values and any notFound names (#591)", "set_cvars", (p) => ({ cvars: p.cvars })),
    ...
    set_viewport: bp("mutate", "Set viewport camera. Params: location?, rotation?", "set_viewport_camera"),
    ...
    get_build_status: bp("mutate", "Get build/map status. Params: none", "get_build_status"),
    ...
    list_dialogs: bp("read", "Read the modal dialog blocking the editor, in full: its exact title, its COMPLETE message text (never truncated - messageTruncated is always false), every button label in the order the dialog lays them out, and a `choices` array pairing each button with the exact editor(respond_to_dialog) call that presses it. No button is marked recommended and none is reordered; choosing is yours. Also reports which armed policy matches and which button that policy would press, if any. Runs even while a dialog is blocking the editor, when every other handler times out, so this is the way to see what the editor is asking. Params: none", "list_dialogs"),
    respond_to_dialog: bp("mutate", "Press one named button on the active modal dialog, releasing the game thread. This is the deliberate way to answer a dialog: read it with editor(list_dialogs) first, then name the button you chose. Runs even while the dialog is blocking the editor. Pass action='close' to destroy the dialog window when no button label fits, which ends the modal without answering the question. Params: buttonIndex?, buttonLabel?, action? (escape or close)", "respond_to_dialog"),
    ...
    list_dirty_packages: bp("mutate", "Enumerate currently-dirty content + map packages, read from the editor's own dirty-package lists (the same ones Save All uses). Includes a never-saved /Temp world, because an unsaved new map is exactly the unsaved work a caller needs to see before closing or reloading. Params: none (#340)", "list_dirty_packages"),
