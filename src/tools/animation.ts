...
export const animationTool: ToolDef = categoryTool(
  ...
  "Animation assets, skeletons, montages, blendspaces, anim blueprints, physics assets.",
  {
    ...
    read_sequence:        bp("read", "Read anim sequence: length, rate scale, frames, root motion, curves and notifies. Each notify carries triggerTime and duration in seconds, trackIndex and trackName, and for a notify state its notifyStateClass and endTime. Resolves the same path forms asset(read) does. Params: assetPath", "read_anim_sequence", (p) => ({ assetPath: p.assetPath })),
    ...
    add_transition:       bp("mutate", "Add directed transition between states. blendDuration and blendLogic are applied to the new transition as set_transition_blend applies them, and the result echoes the effective blendDuration and blendLogic. An existing transition is returned unchanged, with a warning when the requested blend differs. Params: assetPath, stateMachineName, fromState, toState, blendDuration? (seconds, engine default 0.2), blendLogic? (Standard|Inertialization)", "add_transition"),
    ...
    create_control_rig:   bp("mutate", "Create a Control Rig Blueprint from a skeletal mesh or a skeleton through the engine's Control Rig factory, the same path as the Content Browser's Create Control Rig: the source's bone hierarchy is imported and the preview mesh set. Pass exactly one source. name defaults to '<Source>_CtrlRig' and packagePath to the source's folder. onConflict=skip (default) returns the existing asset, error refuses; it never overwrites. Returns a delete_asset rollback. Params: skeletalMeshPath? | skeletonPath?, name?, packagePath?, onConflict? (#1133)", "create_control_rig", (p) => ({ skeletalMeshPath: p.skeletalMeshPath, skeletonPath: p.skeletonPath, name: p.name, packagePath: p.packagePath, onConflict: p.onConflict })),
