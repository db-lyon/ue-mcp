...
export const animationTool: ToolDef = categoryTool(
  ...
  "Animation assets, skeletons, montages, blendspaces, anim blueprints, physics assets.",
  {
    ...
    set_motion_matching_chooser: bp("Drive the Motion Matching node's Database from a ChooserTable so the database is selected at runtime by character state - the actual usable payoff of a pose-search chooser. Wires a thread-safe EvaluateChooser (result typed to PoseSearchDatabase) into the MM node's Database pin, with the chooser's context object defaulting to the anim instance (Self), matching choosers that branch on AnimBP variables. Params: assetPath (AnimBP), chooserPath (ChooserTable), graphName? (default AnimGraph) (motion matching)", "set_motion_matching_chooser", (p) => ({ assetPath: p.assetPath, chooserPath: p.chooserPath, graphName: p.graphName })),
