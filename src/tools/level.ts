...
export const levelTool: ToolDef = categoryTool(
  ...
  "Level actors, selection, components, level management, volumes, lights, and splines.",
  {
    ...
    get_bone_transform: bp("Read a bone or socket transform on a live actor's SkeletalMeshComponent. Wraps GetBoneTransform / GetSocketTransform so callers don't fight the Python opaque-FHitResult issue. Params: actorLabel, boneName (or socket name), componentName? (default: CharacterMesh0 / Mesh / first SK component), space? (world|component|local, default world). Returns location, rotation, scale (#420)", "get_bone_transform", (p) => ({ actorLabel: p.actorLabel, boneName: p.boneName, componentName: p.componentName, space: p.space })),
    list_bones: bp("List bones in an actor's SkeletalMeshComponent ref skeleton (name, index, parent). Params: actorLabel, componentName? (#420)", "list_bones", (p) => ({ actorLabel: p.actorLabel, componentName: p.componentName })),
  ...
  undefined,  // actionDocs auto-generated from descriptions
  {
    ...
    boneName: z.string().optional().describe("Bone or socket name for get_bone_transform"),
    space: z.string().optional().describe("get_bone_transform space: world (default) | component | local"),
    bodyComponent: z.string().optional().describe("rebind_leader_pose: explicit body component name (default CharacterMesh0/Mesh)"),
