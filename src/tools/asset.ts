...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    read_graph: bp("read", "Read the NODE GRAPH of any EdGraph-backed asset: every graph in it, every node, and every node's pins with what each pin is connected to. Reflection cannot reach topology: UEdGraphNode declares its Pins array with no UPROPERTY, and UEdGraphPin is a plain class rather than a UObject, so no set_property, reflect_instance or find_object can read, break or make a connection (#1059). For the EdGraph types with no category of their own, CustomizableObject (Mutable) among them. A Blueprint, a PCG graph or a Material is REFUSED here and pointed at blueprint(read_graph) / blueprint(get_connections) / pcg(read_graph) / material(read_graph), which own those types and address them in their own terms. Each node reports name, path, class, title, position and comment; each pin reports name, pinId, direction, type, defaultValue and linkedTo[] naming the node and pin at the other end. Node settings themselves are read with reflection(action='reflect_instance') on the node path this returns, which already works; this reports what that cannot see. connectionCount is per graph and counts each wire once; nodesReported is how many nodes this call emitted, which maxNodes can cut below a graph's own nodeCount. A node reported with hasNoPins is broken: AllocateDefaultPins never ran for it, so it can never be wired and the editor cannot draw it. Each node also reports nodeGuid. To change the graph use connect_graph_pins, disconnect_graph_pins, add_graph_node and remove_graph_node, which go through the graph's own schema. An asset with no graph answers with an empty list and says so rather than failing. Params: assetPath, graphName? (substring filter), includePins? (default true), maxNodes? (default 500, max 5000)", "read_asset_graph", (p) => ({ assetPath: p.assetPath ?? p.path, graphName: p.graphName, includePins: p.includePins, maxNodes: p.maxNodes })),
    connect_graph_pins: bp("mutate", "Connect two pins in any EdGraph-backed asset with no category of its own, CustomizableObject (Mutable) among them, through the graph's own schema (TryCreateConnection), so the graph type's rules decide what is legal. A refusal returns the schema's own message with reason='schema_disallowed'. Address each end by node (nodeGuid, node path, name or unique title from read_graph) and pinId, which is the reliable form because pin names repeat on some node types; pin name plus direction also works. A pinId alone needs no node. brokenLinks lists any link the schema replaced. Rollback: disconnect_graph_pins. A Blueprint, Material or PCG graph is refused and pointed at its own category. Params: assetPath, graphName?, sourceNode?, sourcePinId?, sourcePin?, sourcePinDirection?, targetNode?, targetPinId?, targetPin?, targetPinDirection?, save? (default true)", "connect_graph_pins", (p) => ({ assetPath: p.assetPath ?? p.path, graphName: p.graphName, sourceNode: p.sourceNode, sourcePinId: p.sourcePinId, sourcePin: p.sourcePin, sourcePinDirection: p.sourcePinDirection, targetNode: p.targetNode, targetPinId: p.targetPinId, targetPin: p.targetPin, targetPinDirection: p.targetPinDirection, save: p.save })),
    disconnect_graph_pins: bp("mutate", "Break links in an EdGraph-backed asset through its schema. With a target pin, breaks that one link (BreakSinglePinLink); without one, breaks every link on the source pin (BreakPinLinks). Pins are addressed as in connect_graph_pins. A link that does not exist is reported with wasConnected=false and nothing changes. Rollback: connect_graph_pins when exactly one link was broken; otherwise brokenLinks names each one. Params: assetPath, graphName?, sourceNode?, sourcePinId?, sourcePin?, sourcePinDirection?, targetNode?, targetPinId?, targetPin?, targetPinDirection?, save? (default true)", "disconnect_graph_pins", (p) => ({ assetPath: p.assetPath ?? p.path, graphName: p.graphName, sourceNode: p.sourceNode, sourcePinId: p.sourcePinId, sourcePin: p.sourcePin, sourcePinDirection: p.sourcePinDirection, targetNode: p.targetNode, targetPinId: p.targetPinId, targetPin: p.targetPin, targetPinDirection: p.targetPinDirection, save: p.save })),
    add_graph_node: bp("mutate", "Add a node to an EdGraph-backed asset's graph through the schema's own node action, so its pins are allocated and it is parented to the graph. nodeClass is the class the action spawns (a CustomizableObjectNode class for Mutable, a SoundNode class for a SoundCue); actionName picks a menu entry by name ('category|name' also works) when several actions spawn the same class. When nothing matches, the error lists availableActions. spawnMode=direct constructs nodeClass itself (CreateNewGuid, PostPlacedNewNode, AllocateDefaultPins), and auto does that only for a schema that publishes no node actions. Returns nodeGuid, nodePath and every pin with its pinId. Rollback: remove_graph_node. Params: assetPath, graphName? (required when the asset has several graphs), nodeClass?, actionName?, spawnMode? (auto|action|direct), posX?, posY?, save? (default true)", "add_graph_node", (p) => ({ assetPath: p.assetPath ?? p.path, graphName: p.graphName, nodeClass: p.nodeClass, actionName: p.actionName, spawnMode: p.spawnMode, posX: p.posX, posY: p.posY, save: p.save })),
    remove_graph_node: bp("mutate", "Remove a node from an EdGraph-backed asset's graph: the schema breaks its links, then the node is destroyed. A node the graph type forbids deleting (a root or output node) is refused. brokenLinks names every link it had so they can be re-made. Rollback: add_graph_node with the same class and position when exactly one schema action spawns it (lossy: a fresh default node); otherwise rollbackPossible=false. Params: assetPath, node (nodeGuid, path, name or unique title), graphName?, save? (default true)", "remove_graph_node", (p) => ({ assetPath: p.assetPath ?? p.path, graphName: p.graphName, node: p.node, save: p.save })),
    create_customizable_object: bp("mutate", "Create a Mutable CustomizableObject through Mutable's own factory, so it holds its Source graph with the Base Object node and can be authored with add_graph_node and connect_graph_pins at once. An asset made with create_asset_by_class has no graph. Returns objectPath, graphName, rootNode (nodeGuid and pins with pinIds) and every graph node. onConflict=skip returns an existing CustomizableObject as existed; error refuses. Fails with 'Mutable plugin not available' when the plugin is off. Rollback: delete_asset. Params: name, packagePath? (default /Game), onConflict? (skip|error, default skip), save? (default true)", "create_customizable_object", (p) => ({ name: p.name, packagePath: p.packagePath, onConflict: p.onConflict, save: p.save })),
    ...
    import_animation:     bp("mutate", "Import anim from FBX. importSettings sets any FbxAnimSequenceImportData or FbxImportUI field by UPROPERTY name or dotted path, e.g. {bSnapToClosestFrameBoundary: true} for clips whose length is not frame-aligned; a bool may drop its b prefix. An unknown key fails before anything is imported (#1134). Params: filePath, name?, packagePath?, skeletonPath, importCustomAttribute?, removeRedundantKeys?, importSettings?", "import_animation", (p) => ({ filename: p.filePath, destinationPath: p.packagePath, assetName: p.name, skeletonPath: p.skeletonPath, importCustomAttribute: p.importCustomAttribute, removeRedundantKeys: p.removeRedundantKeys, importSettings: p.importSettings })),
    import_file:          bp("mutate", "Import any source file through a chosen factory, unattended. factoryClass names a UFactory subclass by name or /Script path, including one a plugin ships (resolved at runtime; omit it to let the editor pick by extension). factoryProperties sets that factory's properties by UPROPERTY name or dotted path before the import runs, which is how a factory that raises its own options dialog is told to use the supplied options instead, since unattended mode cancels the dialog and the import with it. The file extension is checked against the factory's formats first. An unknown property fails before anything is imported (#1096). Params: filePath, packagePath, name?, factoryClass?, factoryProperties?, replaceExisting? (default true), save? (default false), automated? (default true)", "import_file", (p) => ({ filename: p.filePath, destinationPath: p.packagePath, assetName: p.name, factoryClass: p.factoryClass, factoryProperties: p.factoryProperties, replaceExisting: p.replaceExisting, save: p.save, automated: p.automated })),
    ...
    read_cloth_data:      bp("read", "Read Chaos cloth data on a skeletal mesh: per clothing asset, its configs (reflected properties), LOD count, and per-LOD point-weight-map summary (name, target, vertex count, min/max - including the MaxDistances mask). Also reports which render sections each clothing asset is bound to (boundSections, bound), every section of every LOD (sections: lodIndex, sectionIndex, materialIndex, usesCloth, clothingAsset, assetLodIndex) and unboundClothingAssets: an asset bound to no section never simulates. Params: skeletalMeshPath (#595/#1139)", "read_cloth_data", (p) => ({ skeletalMeshPath: p.skeletalMeshPath })),
    bind_cloth_to_section: bp("mutate", "Bind a clothing asset to one render section so it simulates, as the skeletal mesh editor's section clothing dropdown does. Whatever the section was bound to is unbound first. lodIndex and sectionIndex are validated against the mesh, assetLodIndex against the clothing asset. Saves the mesh; rolls back to the previous binding. Params: skeletalMeshPath, lodIndex, sectionIndex, clothingAsset? (name; optional when the mesh has one), assetLodIndex? (default: lodIndex, clamped to the asset's LODs) (#1139)", "bind_cloth_to_section", (p) => ({ skeletalMeshPath: p.skeletalMeshPath, lodIndex: p.lodIndex, sectionIndex: p.sectionIndex, clothingAsset: p.clothingAsset, assetLodIndex: p.assetLodIndex })),
    unbind_cloth_from_section: bp("mutate", "Remove the clothing asset bound to one render section. The engine unbinds per LOD, so other sections of that LOD bound to the same asset are released too and listed in releasedSections. A section with no binding is a no-op. Saves the mesh; rolls back by rebinding. Params: skeletalMeshPath, lodIndex, sectionIndex, clothingAsset? (refuse unless the section is bound to this one) (#1139)", "unbind_cloth_from_section", (p) => ({ skeletalMeshPath: p.skeletalMeshPath, lodIndex: p.lodIndex, sectionIndex: p.sectionIndex, clothingAsset: p.clothingAsset })),
    ...
    read_datatable:       bp("read", "Read DataTable rows. A large table overflows the response inline: pass outputPath to write the rows (after rowFilter) to a file as a JSON array instead. The response then carries outputPath (resolved absolute path), rowCount, bytes, rowStruct and totalRowCount, with no rows or rowNames. A relative outputPath resolves under the project's Saved/, missing folders are created and an existing file is overwritten. Params: assetPath, rowFilter?, outputPath?", "read_datatable", (p) => ({ path: p.assetPath, rowFilter: p.rowFilter, outputPath: p.outputPath })),
    ...
    move_folder:          bp("mutate", "Move/rename entire content folder with redirector fixup in one transaction. Refused with reason=package_objects_left_behind, before anything moves, when a package in the folder holds an object the move would leave at the old path (#1137). Params: sourcePath, destinationPath (#192)", "move_folder"),
  ...
  undefined,
  {
    ...
    graphName: z.string().optional().describe("read_graph: only read graphs whose name contains this. An asset with several graphs (a Blueprint's EventGraph and its functions, a state machine's inner graphs) reports them all without it"),
    includePins: z.boolean().optional().describe("read_graph: include each node's pins and their connections (default true). false gives node names, classes and counts only, which is the cheap shape for a large graph"),
    maxNodes: z.number().optional().describe("read_graph: cap on nodes reported per graph (default 500, max 5000). A graph cut short is marked truncated"),
    sourceNode: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: node of the source pin, by nodeGuid, node path, name or unique title (from read_graph)"),
    sourcePinId: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: pinId of the source pin (from read_graph). Preferred: pin names repeat on some node types"),
    sourcePin: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: source pin name, when no pinId is given"),
    sourcePinDirection: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: input | output, to disambiguate sourcePin"),
    targetNode: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: node of the target pin"),
    targetPinId: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: pinId of the target pin. disconnect_graph_pins without a target breaks every link on the source pin"),
    targetPin: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: target pin name, when no pinId is given"),
    targetPinDirection: z.string().optional().describe("connect_graph_pins / disconnect_graph_pins: input | output, to disambiguate targetPin"),
    nodeClass: z.string().optional().describe("add_graph_node: the class the node action spawns (e.g. a CustomizableObjectNode class, or a SoundNode class for a SoundCue)"),
    actionName: z.string().optional().describe("add_graph_node: schema menu entry name, or 'category|name', when several actions match or no nodeClass is known"),
    spawnMode: z.string().optional().describe("add_graph_node: auto (default) | action | direct"),
    posX: z.number().optional().describe("add_graph_node: node X position in the graph"),
    posY: z.number().optional().describe("add_graph_node: node Y position in the graph"),
    node: z.string().optional().describe("remove_graph_node: the node to remove, by nodeGuid, node path, name or unique title"),
    ...
    skeletalMeshPath: z.string().optional().describe("read_cloth_data/set_cloth_config/bind_cloth_to_section/unbind_cloth_from_section: skeletal mesh path (#595)"),
    clothingAsset: z.string().optional().describe("set_cloth_config: clothing asset name filter (#595). bind_cloth_to_section / unbind_cloth_from_section: the clothing asset by name (#1139)"),
    ...
    replaceExisting: z.boolean().optional().describe("import_skeletal_mesh / import_file: replace an existing asset at the destination (default true) (#678)"),
    importCustomAttribute: z.boolean().optional().describe("import_animation: import FBX custom attributes as curves (default true)"),
    removeRedundantKeys: z.boolean().optional().describe("import_animation: strip keys that do not change the value (default true)"),
    factoryClass: z.string().optional().describe("import_file: UFactory subclass to import with, by class name or /Script/Module.Class path, e.g. VRM4UImporterFactory (#1096)"),
    factoryProperties: z.record(z.unknown()).optional().describe("import_file: properties set on the factory before import, keyed by UPROPERTY name or dotted path, e.g. {bUseProvidedImportOptions: true} (#1096)"),
    importSettings: z.record(z.unknown()).optional().describe("import_animation: FbxAnimSequenceImportData / FbxImportUI overrides keyed by UPROPERTY name or dotted path, e.g. {bSnapToClosestFrameBoundary: true} (#1134)"),
