...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    read_graph: bp("read", "Read the NODE GRAPH of any EdGraph-backed asset: every graph in it, every node, and every node's pins with what each pin is connected to. For a Blueprint prefer blueprint(get_connections), which addresses nodes by GUID; this is the only reader for every other EdGraph-backed type. Reflection cannot reach any of it: UEdGraphNode declares its Pins array with no UPROPERTY, and UEdGraphPin is a plain class rather than a UObject, so no set_property, reflect_instance or find_object can read, break or make a connection (#1059). Works on anything built on the engine's graph type - CustomizableObject (Mutable), Material, AnimBP, Niagara - because they all use UEdGraph. Each node reports name, path, class, title, position and comment; each pin reports name, pinId, direction, type, defaultValue and linkedTo[] naming the node and pin at the other end. Node settings themselves are read with reflection(action='reflect_instance') on the node path this returns, which already works; this reports what that cannot see. connectionCount is per graph and counts each wire once. A node reported with hasNoPins is broken: AllocateDefaultPins never ran for it, so it can never be wired and the editor cannot draw it. READ ONLY: making a connection means running the owning schema's own rules, which is separate work. An asset with no graph answers with an empty list and says so rather than failing. Params: assetPath, graphName? (substring filter), includePins? (default true), maxNodes? (default 500, max 5000)", "read_asset_graph", (p) => ({ assetPath: p.assetPath ?? p.path, graphName: p.graphName, includePins: p.includePins, maxNodes: p.maxNodes })),
