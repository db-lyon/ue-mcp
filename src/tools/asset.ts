...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables.",
  {
    ...
    rename:         bp("Rename asset. Params: assetPath, newName. World Partition levels are detected and their __ExternalActors__/__ExternalObjects__ packages are migrated atomically alongside the .umap (#409). For batches of 3+ scene-referenced non-world assets use bulk_rename instead - each single rename forces a full redirector-fixup pass and can crash the editor.", "rename_asset"),
