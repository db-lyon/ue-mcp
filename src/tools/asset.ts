...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    read_import_sources:  bp("mutate", "Read AssetImportData source filenames on an imported asset (StaticMesh, SkeletalMesh, Texture, Animation, etc.). Returns sources[] of {relativeFilename, absolutePath, timestamp, fileHash, displayLabelName}. Params: assetPath (#270)", "read_import_sources", (p) => ({ assetPath: p.assetPath ?? p.path })),
