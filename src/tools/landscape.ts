...
export const landscapeTool: ToolDef = categoryTool(
  ...
  "Landscape terrain: info, layers, sculpting, weight painting, materials, splines, proxies.",
  {
    ...
    list_edit_layers:  bp("read", "List the landscape's edit layers by name, in stack order, with guid, visible, locked, heightmapAlpha and weightmapAlpha. These names are what sculpt, paint_layer and the region writes take as editLayer. Params: actorLabel? OR actorPath? (#1156)", "list_landscape_edit_layers", (p) => ({ actorLabel: p.actorLabel, actorPath: p.actorPath })),
    merge_edit_layers: bp("mutate", "Re-merge every edit layer into the landscape's final height and weight maps (ALandscape::ForceLayersFullUpdate), then run the update immediately unless updateNow=false. Use after edits that leave the merged result stale. Changes no authored value, so there is no rollback. Params: actorLabel? OR actorPath?, updateNow? (default true) (#1156)", "merge_landscape_edit_layers", (p) => ({ actorLabel: p.actorLabel, actorPath: p.actorPath, updateNow: p.updateNow })),
  ...
  undefined,
  {
    ...
    updateNow: z.boolean().optional().describe("merge_edit_layers: run the merge now rather than on the next editor tick (default true) (#1156)"),
