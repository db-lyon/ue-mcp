...
export const widgetTool: ToolDef = categoryTool(
  ...
  "UMG Widget Blueprints, Editor Utility Widgets, and Editor Utility Blueprints.",
  {
    ...
    add_to_pie_viewport:      bp("(#602) Instantiate a UserWidget Blueprint/class in the active PIE viewport for visual verification. Params: assetPath, playerIndex?, forceVisible?, visibility?, zOrder?", "add_widget_to_pie_viewport", (p) => ({ assetPath: p.assetPath, path: p.path, playerIndex: p.playerIndex, forceVisible: p.forceVisible, visibility: p.visibility, zOrder: p.zOrder })),
  ...
  undefined,
  {
    ...
    playerIndex: z.number().optional().describe("add_to_pie_viewport: PIE player index (default 0)"),
    forceVisible: z.boolean().optional().describe("add_to_pie_viewport: set SelfHitTestInvisible before adding"),
    visibility: z.string().optional().describe("add_to_pie_viewport: explicit Slate visibility"),
    zOrder: z.number().optional().describe("add_to_pie_viewport: viewport z-order (default 0)"),
