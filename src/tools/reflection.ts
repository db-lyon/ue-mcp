...
export const reflectionTool: ToolDef = categoryTool(
  ...
  "UE reflection: classes, structs, enums, gameplay tags, and SaveGame instances.",
  {
    ...
    list_structs:   bp("read", paged("List UScriptStructs, native and user-defined. Rows carry name (the registered spelling, without the F prefix), cppName (with it), path, package, parent, native and tableRow (derives from FTableRowBase, so usable as a DataTable row struct), sorted by path so a page boundary is stable. package narrows to one package or content folder: /Script/Engine, a bare module name (Engine), or /Game/Data. filter is a case-insensitive substring of name or cppName. Params: package?, filter? (#1088)"), "list_structs", (p) => ({ package: p.package, filter: p.filter, cursor: p.cursor, limit: p.limit })),
    list_tags:      bp("read", paged("List gameplay tags. Params: filter?"), "list_gameplay_tags"),
    create_tag:     bp("mutate", "Create a gameplay tag in Config/DefaultGameplayTags.ini. Registers it live through the Gameplay Tags editor when that module is loaded (method=gameplay_tags_editor, registeredLive=true, no restart); otherwise appends the ini entry and the tag appears after a restart (method=ini_append). Idempotent: a tag the ini already declares reports existed. A failure names the ini path and the cause, including a read-only (checked-in) file. Params: tag, comment?", "create_gameplay_tag"),
  ...
  undefined,
  {
    ...
    package: z.string().optional().describe("list_structs: /Script/<Module> package, bare module name, or content folder to narrow to"),
