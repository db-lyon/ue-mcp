...
export const gameplayTool: ToolDef = categoryTool(
  ...
  "Gameplay systems: physics, collision, navigation, input, behavior trees, AI (EQS, perception, State Trees, Smart Objects), game framework.",
  {
    ...
    pie_record_arm:         bp("Arm a PIE input recording for the next BeginPIE (or immediately if PIE is already running). Writes manifest.json / sequence.json / recording.csv into Saved/MCPRecordings/<id>/ on EndPIE. Params: actions? (whitelist of UInputAction paths; empty = record all bound), track_values? (dotted reflection paths sampled to doubles per frame), axis_threshold? (default 0.15), sample_hz? (default 60), pin_fps? (default = sample_hz; 0 to skip the t.MaxFPS pin), capture_pawn_state? (default true), capture_montage? (default true), rng_seed? (default auto), run_gap_frames? (default 6), recording_dir? (override of Saved/MCPRecordings/), id? (override the auto-generated id). Determinism note: pins FMath::RandInit and t.MaxFPS but does not affect FRandomStream or Niagara/AI RNG.", "pie_record_arm", (p) => ({ actions: p.actions, track_values: p.track_values, axis_threshold: p.axis_threshold, sample_hz: p.sample_hz, pin_fps: p.pin_fps, capture_pawn_state: p.capture_pawn_state, capture_montage: p.capture_montage, rng_seed: p.rng_seed, run_gap_frames: p.run_gap_frames, recording_dir: p.recording_dir, id: p.id })),
    ...
    pie_record_read:        bp("Read a recording artifact. Params: id, file? (manifest|sequence|csv|drift, default manifest), recording_dir?, record_limit? (CSV rows; default 1000), record_offset?.", "pie_record_read", (p) => ({ id: p.id, file: p.file, recording_dir: p.recording_dir, limit: p.record_limit, offset: p.record_offset })),
    ...
    pie_replay_arm:         bp("Arm a PIE replay for the next BeginPIE (or current PIE). Drives the recorded sequence through UEnhancedInputLocalPlayerSubsystem::InjectInputForAction (input/hold/input_tape) plus console/mark/capture steps. Writes drift.json (per-frame pawn delta vs source) when replaying a known recording_id. Params: recording_id? (loads sequence.json + recording.csv for drift), sequence_path? (explicit), steps? (inline array; same schema as sequence.json), settle_ms?, sample_hz?, pin_fps? (default = sample_hz; 0 to skip), apply_rng_seed? (default true), record_drift? (default true), auto_stop_pie? (default false), drift_thresholds? ({ position_cm?, rotation_deg?, velocity_cms? } for over-threshold list), rng_seed? (inline-only).", "pie_replay_arm", (p) => ({ recording_id: p.recording_id, sequence_path: p.sequence_path, steps: p.steps, recording_dir: p.recording_dir, settle_ms: p.settle_ms, sample_hz: p.sample_hz, pin_fps: p.pin_fps, apply_rng_seed: p.apply_rng_seed, record_drift: p.record_drift, auto_stop_pie: p.auto_stop_pie, drift_thresholds: p.drift_thresholds, rng_seed: p.rng_seed })),
