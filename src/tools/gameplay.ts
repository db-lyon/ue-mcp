...
export const gameplayTool: ToolDef = categoryTool(
  ...
  "Gameplay systems: physics, collision, navigation, input, behavior trees, AI (EQS, perception, State Trees, Smart Objects), game framework.",
  {
    ...
    add_impulse:            bp("Apply an impulse (or force with mode='force') to a (PIE) actor's simulating physics body so its motion can be observed over time - loop level(read_actor_motion) to sample the response. Params: actorLabel, impulse {x,y,z} (or force/vector), mode? (impulse|force), componentName?, boneName?, location? (apply at world point), velChange?/accelChange?, world? (editor|pie|auto) (#676)", "add_impulse", (p) => ({ actorLabel: p.actorLabel, impulse: p.impulse, force: p.force, vector: p.vector, mode: p.mode, componentName: p.componentName, boneName: p.boneName, location: p.location, velChange: p.velChange, accelChange: p.accelChange, world: p.world })),
    ...
    set_mapping_modifiers:  bp("Set modifiers/triggers on an IMC mapping. Each modifier is either {type:'<ShortName>', <prop>:<val>} or {class:'/Script/Module.Class', properties:{...}} - the class form resolves any custom UInputModifier subclass (#649). Params: imcPath, mappingIndex?, modifiers?, triggers?", "set_mapping_modifiers"),
